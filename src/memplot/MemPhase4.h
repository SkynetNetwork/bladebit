...
inline void WriteP7Entries( const uint64 length, const uint32* indices, byte* parkBuffer )
{
    ...
    // skynetpos requires this to have an extra bit for some odd reason.
                                ...
                                const uint32* f7Entries, uint32* parkBuffer )
{
    ...
    {
        // #NOTE: Unfortunately, skynetpos infers the size of the C2 table by substracting
        ...
        //  This will force skynetpos to stop at that point as the f7 is lesser than max k32 value.
    ...
    else
    {
        ...
        // Write an empty one at the end (compatibility with skynetpos)
